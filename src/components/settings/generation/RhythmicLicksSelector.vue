...
const useLicks = computed({
  ...
  set: (value) => generationStore.setUseRhythmicLicks(value ?? false)
})

const frequency = computed({
  get: () => generationStore.rhythmicLickFrequency,
  set: (value) => generationStore.setRhythmicLickFrequency(value ?? 0)
})
</script>
  ...
  <div class="flex flex-col gap-4">
    ...
    <div v-if="useLicks" class="flex flex-col justify-around gap-2 mt-2">
      ...
      <div class="flex items-center gap-4 w-full mt-2">
        ...
        <span class="w-10 text-right text-xs tabular-nums">{{ ((frequency ?? 0) * 100).toFixed(0) }}%</span>
      </div>
    </div>
  </div>
</template>
